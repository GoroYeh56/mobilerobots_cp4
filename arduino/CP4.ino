...
int sampling_time = 1000; // 1 second.
int time = 0;
int last_time = 0;
int num_of_zeros = 0;
int num_of_ones = 0;
...
void KEEP_FIND_BEACON(){
    ...
    time = millis();
    if(time - last_time >= sampling_time ){
        // compute ratio.
        ratio = num_of_zeros / (num_of_ones+num_of_zeros);
        Serial.println(ratio);
        // Beacon 1: 600 => 0.27   0.32
        // Beacon 2: 1500 => 0.17 ~ 0.22
        if(ratio >= 0.25){
